...
Install with `npx -y thoughtproof-mcp@0.3.0`. Works with **Claude Desktop**, **Cursor**, **Windsurf**, **Cline**, and other local stdio MCP clients.
...
Verify any claim or AI-generated reasoning via RV (`POST /v1/check`). Not the hero tool.
...
|---------------------|---------|-------------|
| `DQL_API_KEY` | *(none)* | DQL verify key (`dqlk_…`) for `verify_decision`. Alias: `THOUGHTPROOF_DQL_KEY` |
